...
mycon=m.connect(host='localhost',user='Neha',passwd='MySql',database='miniproject')
...
img1=Image.open(r'C:\Users\dprab\Downloads\docimg.png')
img2=Image.open(r'C:\Users\dprab\Downloads\adimg.png')
img3=Image.open(r'C:\Users\dprab\Downloads\patimg.png')
img4=Image.open(r'C:\Users\dprab\Downloads\backimg.png')
    ...
    if user=='Doctor Login':
        def operation(op_name):
            if op_name=='upd':
                        ...
                        P_id_label=Label(frame3,text='Enter Patient ID to update prescriptions for:',font=('Lucida Bright',15,'italic','underline'),background='#8B8B7D',foreground='#F5F5F5').grid(row=2,column=0,pady=10,padx=10)
            ...
            if op_name=='show':
                def doctor_show():
                            ...
                            #print(password)
                            cursor.reset()
                            s = "select * from doctor where Doctor_ID='{}'".format(username,)
                            cursor.execute(s)
                            result1=cursor.fetchall()
                            #print(result1);print('--------------------------')
                            if result1:
                                patient_tree_window = Toplevel()
                                frame = Frame(patient_tree_window)
                                frame.pack()
                                
                                tree_scroll1= Scrollbar(frame,orient='horizontal')
                                tree_scroll2= Scrollbar(frame,orient='vertical')
                                tree_scroll1.pack(side=BOTTOM,fill=X)
                                tree_scroll2.pack(side=RIGHT,fill=BOTH)

                                # Create Treeview
                                tree = ttk.Treeview(frame, xscrollcommand=tree_scroll1.set,selectmode='browse')
                                tree = ttk.Treeview(frame, yscrollcommand=tree_scroll2.set,selectmode='browse')
                                # Pack to the screen
                                tree.pack()
                                
                        #         tree_scroll = Scrollbar(tree)
                        #         tree_scroll.pack(side=RIGHT, fill=X)

                                #Configure the scrollbar
                                tree_scroll1.configure(command=tree.xview)
                                tree_scroll2.configure(command=tree.yview)

                                
                                
                                tree['columns'] = ('Doc_ID', 'Name', 'Age', 'Specialization', 'Phone Number', 'Email', 'Patient ID','Patient Name','Diagnosis', 'Treatment','Status')
                                tree.column('#00',minwidth=10)
                                tree.column('Doc_ID', anchor=W)
                                tree.column('Name', anchor=W)
                                tree.column('Age', anchor=W)
                                tree.column('Specialization', anchor=W)
                                tree.column('Phone Number', anchor=W)
                                tree.column('Email', anchor=W)
                                tree.column('Patient ID', anchor=W,width=0)
                                tree.column('Patient Name', anchor=W)
                                tree.column('Diagnosis', anchor=W)
                                tree.column('Treatment', anchor=W)
                                tree.column('Status', anchor=W)

                                tree.heading('#0', text='Label')
                                tree.heading('Doc_ID', text='ID')
                                tree.heading('Name', text='Doctor_name')
                                tree.heading('Age', text='Age')
                                tree.heading('Specialization', text='Specialization')
                                tree.heading('Phone Number', text='Phone_number')
                                ...
                                for i in range(len(result1)):
                                    tree.insert(parent='', index='end', iid=i, text=i+1, values=result1[i])
                                    tree.pack()
                doctor_show()
        ...
        def adm(update_for):
            if update_for=='doc':
                ...
                def operation(op_name):
                    if op_name=='upd':
                        clear((head_label,frame3),(welcome_label,logout_bt))
                        D_id_label=Label(frame3,text='Enter Doctor ID to update for:',font=('Lucida Bright',15,'italic','underline'),background='#8B8B7D',foreground='#F5F5F5').grid(row=2,column=0,pady=10,padx=10)
            ...
            if update_for=='pat':
                ...
                def operation(op_name):
                    if op_name=='upd':
                        clear((head_label,frame3),(welcome_label,logout_bt))
                        P_id_label=Label(frame3,text='Enter Patient ID to update for:',font=('Lucida Bright',15,'italic','underline'),background='#8B8B7D',foreground='#F5F5F5').grid(row=2,column=0,pady=10,padx=10)
                    ...
                    if op_name=='rem':
                        ...
                        def rem():
                            ...
                            if P_id in list(set(map(lambda i:i[0],data_pat))):
                                ...
                                mycon.commit()
                                messagebox.showinfo("Update", "Removal of patient sucessful!!")
                                cursor.reset()
                                cursor.execute('select * from patient')
                                data_pat=cursor.fetchall()
                                cursor.reset()
                    ...
                    if op_name=='add':
                        ...
                        def add():
                            ...
                            def add2():
                                ...
                                mycon.commit()
                                messagebox.showinfo("Update", "Addition of neww patient sucessful!!")
                                cursor.reset()
                                cursor.execute('select * from patient')
                                data_pat=cursor.fetchall()
                                cursor.reset()
                    ...
                    if op_name=='show':
                        ...
                        #         tree_scroll.pack(side=RIGHT, fill=X)
                                ...
                                tree['columns'] = ('ID', 'Patient', 'Age', 'Gender', 'Phone Number', 'Doctor Name', 'Diagnosis', 'Treatment', 'Fee', 'Pincode','Status')
                                tree.column('#00',minwidth=10)
                                tree.column('ID', anchor=W)
                                tree.column('Patient', anchor=W)
                                tree.column('Age', anchor=W)
                                tree.column('Gender', anchor=W)
                                tree.column('Phone Number', anchor=W)
                                tree.column('Doctor Name', anchor=W)
                                tree.column('Diagnosis', anchor=W)
                                tree.column('Treatment', anchor=W)
                                tree.column('Fee', anchor=W,width=0)
                                tree.column('Pincode', anchor=W)
                                tree.column('Status', anchor=W)

                                tree.heading('#0', text='Label')
                                tree.heading('ID', text='ID')
                                tree.heading('Patient', text='Patient_name')
                                tree.heading('Age', text='Age')
                                tree.heading('Gender', text='Gender')
                                tree.heading('Phone Number', text='Phone_number')
                                tree.heading('Doctor Name', text='Doctor_name')
                                tree.heading('Diagnosis', text='Diagnosis')
                                tree.heading('Treatment', text='Treatment')
...
#completely done DONE for adm adm
                    ...
                    if op_name=='add_admin':
                        ...
                        def add():
                            ...
                            if A_id not in list(set(map(lambda i:i[0],data_adm))):
                                ...
                                cursor.reset()
                                cursor.execute('select * from admin')
                                data_adm=cursor.fetchall()#---> to reflect change, i retrieve data again
                                #print(list(set(map(lambda i:i[0],data_adm))))
                                adm('adm')
                            else:
                                messagebox.showerror("ID", " ID already exists. Enter different one.")
    ...
    if user=='Patient Login':
        def patient_details():
                            #print(username)
                            #print(password)
                            s = "select * from patient where Patient_ID='{}' and Name='{}'".format(username,password)
                            cursor.execute(s)
                            result1=cursor.fetchall()
                            print(result1)
                            if result1:
                                ...
                                tree_scroll = Scrollbar(frame,orient='horizontal')
                                tree_scroll.pack(side=BOTTOM,fill=X)

                                # Create Treeview
                                tree = ttk.Treeview(frame, xscrollcommand=tree_scroll.set,selectmode='browse')
